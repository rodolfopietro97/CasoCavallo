...
from Utils.CasoCavalloConstants import MERGER_BUFFER_SIZE
...
def merger_worker(redis_client, redis_queues):
    ...
    while True:
        ...
        for redis_queue in redis_queues:
            # Current random to define with respect to strategy to uses
            # For example: for random bytes uses XOR
            current_random = 0
            ...
            if redis_client.llen(redis_queue['name']) > 0:
                ...
                # Set current random dependently by datatype
