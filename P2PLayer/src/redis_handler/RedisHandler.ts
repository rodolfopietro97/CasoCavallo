...
export class RedisHandler {
    ...
    /**
     * Push a string for a specific list
     * 
     * @param topic Topic of message
     * @param message Messgae of topic
     */
     setList(topic: string, message: string): void {
        // Push message with timestamp
        this.redisClient.rpush(topic, message)
    }
    ...
    /**
     * Get current random of a queue
     * 
     * @param queue Current ranndom of queue to read
     * @returns Messages of topic
     */
    async getCurrentRandom(queue: string): Promise<string> {
        let current = await this.redisClient.get(queue)
